...
def est3_mpc(mpc, v, iterations, target):
    ...
    for it in range(iterations):
        ...
        print(time.time()-pipo)
        print(time.process_time()-pipo_cpu)
        print('accc', np.mean(np.abs((mpc.reconstruct(y) > 0.5) - target)))
...
def function(mpc, v):
    ...
    a = est3_mpc(mpc, v.transpose(), 2, target)
    print('-------clear--------')
    b = est3_clear(v_clear.transpose(), 2, target)
    ...
    x = np.argsort(x)
    y = np.argsort(y)
    print('trust', np.sum(x == y), np.sum(x != y))
    ...
    x = np.argsort(x)
    y = np.argsort(y)
    print('difficulty', np.sum(x == y), np.sum(x != y))
